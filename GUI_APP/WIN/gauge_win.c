...
void gauge_win_create(lv_obj_t *parent)
{
	...
	lv_gauge_set_range(gauge1, 0, 10); // 设置仪表盘的范围
	...
	lv_gauge_set_value(gauge1, 0, SportsInfo.walkspeed);	// 设置指针1指向的数值,我们把指针1当作速度指针
	lv_gauge_set_value(gauge1, 1, 6);						// 设置指针2指向的数值,就让它指向关键数值点
	lv_gauge_set_critical_value(gauge1, 6);					// 设置关键数值点
	...
	lv_label_set_text(label1, "0 m/s");					  // 设置文本
	...
	lv_lmeter_set_range(lmeter1, 0, 10);						   // 设置范围
	lv_lmeter_set_value(lmeter1, SportsInfo.walkspeed);			   // 设置当前的值
